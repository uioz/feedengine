import type {EventHookOn, Fn, MaybeComputedRef, Stoppable} from '@vueuse/core';
import {
  containsProp,
  createEventHook,
  resolveRef,
...
export function useFetch<T>(
  ...
  watch([refetch, resolveRef(url)], ([refetch]) => refetch && execute(), {deep: true});
    ...
    return (payload?: unknown, payloadType?: string) => {
      if (!isFetching.value) {
        ...
        if (isRef(config.payload)) {
          watch([refetch, resolveRef(config.payload)], ([refetch]) => refetch && execute(), {
            deep: true,
          });
        }
